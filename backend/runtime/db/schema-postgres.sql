-- Guardian AI Platform — Postgres schema
-- Used in APP_MODE=local or APP_MODE=dev when DB_TYPE=postgres.
...
CREATE TABLE IF NOT EXISTS environment_tool_deployments (
  ...
  created_at                   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at                   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (logical_tool_definition_id, environment_id)
);
...
CREATE TABLE IF NOT EXISTS project_tool_grants (
  ...
  UNIQUE (project_id, logical_tool_definition_id)
);
...
ALTER TABLE agents ADD COLUMN IF NOT EXISTS authored_via TEXT;
ALTER TABLE agents ADD COLUMN IF NOT EXISTS risk_tier TEXT NOT NULL DEFAULT 'medium';
...
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS agent_core_endpoint_id  TEXT;
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS agent_core_endpoint_arn TEXT;
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS runtime_name            TEXT;
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS s3_code_location        TEXT;
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS deployment_logs         TEXT;
ALTER TABLE agent_environment_deployments ADD COLUMN IF NOT EXISTS error_message           TEXT;
